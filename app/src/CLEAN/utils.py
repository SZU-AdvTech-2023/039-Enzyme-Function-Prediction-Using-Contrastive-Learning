...
# 设置随机种子（random seed），以确保在每次运行时获得相同的随机结果。
def seed_everything(seed=1234):
    random.seed(seed)
    np.random.seed(seed)
    os.environ['PYTHONHASHSEED'] = str(seed)  # 设置 Python 的哈希种子，用于控制字典等数据结构中元素的顺序
    torch.manual_seed(seed)  # 设置 PyTorch 的随机种子，用于生成随机数。
    torch.cuda.manual_seed(seed)
    torch.backends.cudnn.deterministic = True

# 用于从给定的 CSV 文件中获取一个包含两个字典的数据结构。第一个字典 id_ec 是将 ID 映射到一个由分号分隔的 EC 列表，第二个字典 ec_id 是将 EC 映射到对应的 ID 集合。
